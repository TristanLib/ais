#!/bin/bash
# Script to train Graph Neural Network (GNN) model for ship trajectory prediction
...
from models.gnn import create_gnn_model, GNNLoss
...
model = create_gnn_model(config, 'basic')
...
loss_fn = GNNLoss(
    ...
    config['model']['training'].get('smoothness_weight', 0.01)
)
...
# Warmup epochs
warmup_epochs = config['model']['training'].get('warmup_epochs', 10)
...
for epoch in range(max_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'epoch': epoch,
            'val_loss': val_loss
        }, '$OUTPUT_DIR/models/gnn_best.pth')
    else:
        patience_counter += 1
...
print(f"GNN training completed in {training_time:.2f} seconds")

# Load best model for evaluation
checkpoint = torch.load('$OUTPUT_DIR/models/gnn_best.pth')
...
gnn_predictions = np.concatenate(all_predictions, axis=0)
gnn_targets = np.concatenate(all_targets, axis=0)

print(f"GNN inference time: {inference_time:.2f} seconds")
...
gnn_results = metrics_calc.evaluate_model(gnn_predictions, gnn_targets)
...
# Save results
results_path = '$OUTPUT_DIR/models/gnn_results.json'
...
print(f"Saved GNN results to: {results_path}")
...
ax1.set_ylabel('Loss')
ax1.set_title('GNN Training History')
ax1.legend()
ax1.grid(True)
...
plt.tight_layout()
plt.savefig('$OUTPUT_DIR/figures/gnn_training_history.png', dpi=300, bbox_inches='tight')
plt.close()

print("Saved training history plot")

# Export model for deployment
torch.jit.save(torch.jit.script(model.cpu()), '$OUTPUT_DIR/models/gnn_scripted.pt')
...
echo ""
echo "Next steps:"
echo "1. Train STT model: bash scripts/22_train_stt.sh"
echo "2. Train PINN model: bash scripts/23_train_pinn.sh"
echo "3. Evaluate all models: bash scripts/30_eval_all.sh"