#!/bin/bash
# Script to train Spatio-Temporal Transformer (STT) model for ship trajectory prediction

set -e  # Exit on any error

echo "=== Training STT Model ==="
echo "Starting STT model training..."
...
from models.stt import create_stt_model, STTLoss
...
model = create_stt_model(config, 'basic')
...
loss_fn = STTLoss(
    ...
    config['model']['training'].get('spatial_consistency_weight', 0.01),
    config['model']['training'].get('temporal_smoothness_weight', 0.005)
)
...
# Warmup epochs
warmup_epochs = config['model']['training'].get('warmup_epochs', 8)
...
# Gradient clipping
gradient_clip_val = config['model']['training'].get('gradient_clip_val', None)

# Training loop
print("Starting STT training...")
...
for epoch in range(max_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'epoch': epoch,
            'val_loss': val_loss
        }, '$OUTPUT_DIR/models/stt_best.pth')
    else:
        patience_counter += 1
...
print(f"STT training completed in {training_time:.2f} seconds")

# Load best model for evaluation
checkpoint = torch.load('$OUTPUT_DIR/models/stt_best.pth')
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()

# Evaluate model
print("Evaluating STT model...")
all_predictions = []
all_targets = []
...
stt_predictions = np.concatenate(all_predictions, axis=0)
stt_targets = np.concatenate(all_targets, axis=0)

print(f"STT inference time: {inference_time:.2f} seconds")
print(f"STT predictions shape: {stt_predictions.shape}")
...
stt_results = metrics_calc.evaluate_model(stt_predictions, stt_targets)

print("STT Model Results:")
for metric, value in stt_results['overall_metrics'].items():
    print(f"  {metric.upper()}: {value:.4f}")

# Save results
results_path = '$OUTPUT_DIR/models/stt_results.json'
...
print(f"Saved STT results to: {results_path}")
...
ax1.set_ylabel('Loss')
ax1.set_title('STT Training History')
ax1.legend()
ax1.grid(True)
...
plt.tight_layout()
plt.savefig('$OUTPUT_DIR/figures/stt_training_history.png', dpi=300, bbox_inches='tight')
plt.close()

print("Saved training history plot")

# Export model for deployment
torch.jit.save(torch.jit.script(model.cpu()), '$OUTPUT_DIR/models/stt_scripted.pt')
print("Saved TorchScript model for deployment")

EOF

echo "=== STT Training Complete ==="
echo "Models saved to: $OUTPUT_DIR/models/"
echo "Results saved to: $OUTPUT_DIR/models/"
echo "Figures saved to: $OUTPUT_DIR/figures/"
echo ""
echo "Next steps:"
echo "1. Train PINN model: bash scripts/23_train_pinn.sh"
